 function [ p, sp ] = ED4c_linfitxy_MS( xdatain, ydatain, xerrin, yerrin, varargin)

% ----------------------------------------------------------------------- % 
% Madison Shankle has made edits on:
%
% Line 269: % ---- Fitting WHEN error estimation over XDATA and YDATA are given ---- %
%  >> Line 376: % Now plotting
...
%   plot(xdata,ydata,'kx')  % MS edits start here
    ...
    refs = {'Henehan16'; 'Henehan16'; 'Henehan16'; 'Henehan16'; 'Henehan16'; ...
        'Henehan16'; 'Henehan16'; 'Henehan16'; 'Henehan16'; 'Henehan16'; ...
        'Henehan16'; 'Henehan16'; 'Henehan16'; 'Henehan16'; 'Henehan16'; ...
        'Henehan16'; 'Henehan16'; 'Henehan16'; ...
        'Raitzsch18'; 'Raitzsch18'; 'Raitzsch18'; 'Raitzsch18'; 'Raitzsch18'; ...
        'Raitzsch18'; 'Raitzsch18'; 'Raitzsch18'; 'Raitzsch18'; 'Raitzsch18'; ...
        'Raitzsch18'; 'Raitzsch18'; 'Raitzsch18'; ...
        'Guillermic20'; 'Guillermic20'; 'Guillermic20'; 'Guillermic20'; 'Guillermic20'};
    
    types = {'Core-top'; 'Core-top'; 'S.O. TOW'; 'Core-top';  'S.O. TOW'; 'S.O. TOW';...
         'Core-top'; 'S.O. TOW';  'S.O. TOW'; 'S.O. TOW'; 'Core-top'; 'S.O. TOW'; ...
         'SED TRAP'; 'Core-top'; 'SED TRAP'; ; 'Core-top'; 'Core-top'; 'Core-top'; ...
         'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'; ...
         'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'; ...
         'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'; 'Core-top'};
...
% % %         % Henehan16 data
% % %         if strcmp(refs{jj}, 'Henehan16') % Henehan16 coretops will be first
% % %             if strcmp(types{jj}, 'Core-top')
% % %                 if jj == 1
% % %                     scatter(xdata(jj), ydata(jj), mkr_size, 'o', 'MarkerFaceColor', ...
% % %                         default_blue, 'MarkerEdgeColor', default_blue);
% % %                     hold on
% % %                 else
% % %                     scatter(xdata(jj), ydata(jj), mkr_size, 'o', 'MarkerFaceColor', ...
% % %                         default_blue, 'MarkerEdgeColor', default_blue, ...
...
% % %                 end
% % %             end
% % %             if strcmp(types{jj}, 'S.O. TOW') % Hen16 net tows blue triangles
% % %                 if jj == 3 % 3 is first row with S.O. TOW in the list
% % %                     scatter(xdata(jj), ydata(jj), mkr_size, '^', 'MarkerFaceColor', ...
% % %                         'none', 'MarkerEdgeColor', default_blue, 'LineWidth',1.5);
% % %                     hold on
% % %                 else
% % %                     scatter(xdata(jj), ydata(jj), mkr_size, '^', 'MarkerFaceColor', ...
% % %                         'none', 'MarkerEdgeColor', default_blue, 'LineWidth',1.5, ...
...
% % %                 end
% % %             end
% % %             if strcmp(types{jj}, 'SED TRAP') % blue squares empty
% % %                 if jj == 13 % first row with SED TRAP in the list
% % %                     scatter(xdata(jj), ydata(jj), mkr_size+10, 's', 'MarkerFaceColor', ...
% % %                         'none', 'MarkerEdgeColo', default_blue, 'LineWidth',1.5);
% % %                     hold on
% % %                 else
% % %                     scatter(xdata(jj), ydata(jj), mkr_size+10, 's', 'MarkerFaceColor', ...
% % %                         'none', 'MarkerEdgeColo', default_blue, 'LineWidth',1.5, ...
...
% % %         end
% % %         % Raitzsch18
% % %         if strcmp(refs{jj}, 'Raitzsch18')
% % %             if jj == 19 % 1st row with Raitzsch data
% % %                 scatter(xdata(jj), ydata(jj), mkr_size, 'o', 'MarkerFaceColor', ...
% % %                     default_orange, 'MarkerEdgeColor', default_orange);
% % %                 hold on
% % %             else
% % %                 scatter(xdata(jj), ydata(jj), mkr_size, 'o', 'MarkerFaceColor', ...
% % %                     default_orange, 'MarkerEdgeColor', default_orange, ...
...
% % %         end
% % %         % Guillermic20
% % %         if strcmp(refs{jj}, 'Guillermic20')
% % %             if jj == 32 % 1st row iwth Raitzsch data
% % %                 scatter(xdata(jj), ydata(jj), mkr_size, 'o', 'MarkerFaceColor', ...
% % %                     default_green, 'MarkerEdgeColor', default_green);
% % %                 hold on
% % %             else
% % %                 scatter(xdata(jj), ydata(jj), mkr_size, 'o', 'MarkerFaceColor', ...
% % %                     default_green, 'MarkerEdgeColor', default_green, ...
...
% % %     end
  ...
  hold on
% % %   for i=1:length(xdata)   % MS: this does error bars
% % %     x0=xdata(i);
% % %     x1=x0-xerr(i);
% % %     x2=x0+xerr(i);
% % %     y0=ydata(i);
...
% % %   end
  ...
  if not(onlyplot)
  xplot = min(xdata)+(max(xdata)-min(xdata)).*(0-0.2:1./(nhull-1):1.+0.2);  
% % %   % MS (1) !!!: shaded region for line of best fit (added by MS)
% % %   patch( [xplot fliplr(xplot)], [saxipbi(low_alpha,:), fliplr(saxipbi(high_alpha,:))], ...
% % %     'r', 'FaceAlpha',0.25, 'EdgeColor', 'none');
% % %   % MS (2) !!!: solid line of best fit, red
% % %   plot([min(xplot),max(xplot)],p(1)*[min(xplot),max(xplot)]+p(2),'r') 
  ...
  if plothull
    ...
    plot([min(xplot),max(xplot)],p(1)*[min(xplot),max(xplot)]+p(2),'r', ...
        'LineWidth', 1.5) 
    % MS (3) !!!: dashed red upper and lower bounds
    plot(xplot,saxipbi(low_alpha,:),'r--', 'LineWidth', 1.5)
    plot(xplot,saxipbi(high_alpha,:),'r--', 'HandleVisibility', 'off', ...
        'LineWidth', 1.5)
    
% MS (4) !!!: all  my scatter plot points
    for jj = 1:length(xdata)
        % Henehan16 data
        if strcmp(refs{jj}, 'Henehan16') % Henehan16 coretops will be first
            if strcmp(types{jj}, 'Core-top')
            ...
            if strcmp(types{jj}, 'SED TRAP') % blue squares empty
                if jj == 13 % first row with SED TRAP in the list
                    scatter(xdata(jj), ydata(jj), mkr_size+10, 's', 'MarkerFaceColor', ...
                        'none', 'MarkerEdgeColo', default_blue, 'LineWidth',1.75);
                    hold on
                else
                    scatter(xdata(jj), ydata(jj), mkr_size+10, 's', 'MarkerFaceColor', ...
                        'none', 'MarkerEdgeColo', default_blue, 'LineWidth',1.75, ...
...
% % %   % MS: add annotation of best fit line:
% % %   dim = [0.621 0.123 0.280 0.0729]; % [0.5 0.001 0.1 0.3]; % [x y w h]
% % %   annotation('textbox', dim, 'String', FitResultString, 'FitBoxToText', ...
% % %       'off', 'EdgeColor', 'none', 'Color', 'r');
