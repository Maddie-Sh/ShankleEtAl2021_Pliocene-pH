% EXTENDED DATA 6d: pH Timeseries from dissolution-corrected SSTs
% Madison Shankle
% 29-09-2021

% Plots a timeseries of pH, with pH as calculated from SSTs from this
% study's Mg/Ca data with a dissolution correction applied (Regenber et al.
% 2014).

% Reference:
%  Regenberg, M. et al. Global dissolution effects on planktonic 
%  foraminiferal Mg/Ca ratios controlled by the calcite‐saturation state of 
%  bottom waters. Paleoceanography 29, 127–142 (2014).
...
east = xlsread('ED6_Shankle_east_pH_results.xls');
...
% pH calculated from dissolution-corrected SSTs
load('ED6d_Shankle_pH_from_disslnSSTs.mat');
% This: loads "pH_final" variable: 12 western points followed by 12 eastern
% points In correct order to be used with the ages (1st col) of "east" and
% "west" already loaded into this sheet.
...
% Plot pHs calculated using dissolution-corrected SSTs
    % West
    plot(west(:,1), pH_final(1:12, 1), 's', 'MarkerFaceColor', 'y', 'MarkerEdgeColor', 'k');
    
    % East
    plot(east(:,1), pH_final(13:end, 1), 's', 'MarkerFaceColor', 'y', 'MarkerEdgeColor', 'k');
